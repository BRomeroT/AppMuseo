...
(function (c) {
    function q(e, j) {
        this.defaults = {
            ...
            selectedItemChangedEventName: "selectedItemChanged",altoPanorama:"230px"
        }; if (!c(e).data("panorama")) {
            c(e).css({ "-ms-content-zooming": "none", "-ms-touch-action": "none" }); this.opts = c.extend({}, this.defaults, j); this.opts = c.extend(this.opts, {
                currentIndex: 0, busy: !1, _item_width: 0, eventsSettings: { useSetReleaseCapture: !1, lastXYById: {}, moved: 0, yDirection: null }, debug: !0 == this.opts.debug && c("<div>").css({
                    position: "fixed", bottom: "0px", right: "0px", background: "white", border: "1px solid red", overflow: "hidden", width: "0px", height: "0%",
                    zIndex: "9999", color: "black"
                }).appendTo("body")
            }); var g = c(window).width() / 100; this.opts.waitXpercentage = Math.round(g * this.opts.waitXpercentage); this.opts.waitYpercentage = Math.round(g * this.opts.waitYpercentage); this.opts.backPointPercentage = Math.round(g * this.opts.backPointPercentage); 0 == parseInt(this.opts.totalNumberOfItems) && (this.opts.totalNumberOfItems = c(e).children(this.opts.itemsContainerSelector).children(this.opts.itemSelector).length); 0 == parseInt(this.opts.backgroundAnimationStep) && (this.opts.backgroundAnimationStep =
            Math.round(100 / this.opts.totalNumberOfItems)); this.$el = c(e); c("<div>").addClass(this.opts.clickedWrapperClass).css({ zIndex: this.opts.clickedWrapperZindex, opacity: "0", width: c(window).width(), height: this.opts.altoPanorama /*"230px"*/ /*c(window).height()*/, position: "fixed", left: "0px", top: "0px", background: "black", overflow: "scroll" }).prependTo(this.$el); this.opts.debug && (g = c("<div>").css({ position: "fixed", bottom: "0px", left: "0px", background: "white", border: "1px solid red", width: "0px", height: "0%", color: "black" }).appendTo("body"), c("<h2>").html('<a href="#" onclick="$(\'.' +
            ...
            function () { return this.$el.children(this.opts.itemsContainerSelector) }; this.getItems = function () { return this.getItemsContainer().children(this.opts.itemSelector) }; this.getItem = function (b) { return this.getItemsContainer().children(this.opts.itemSelector + ":eq(" + b + ")") }; this.getLastItem = function () { return this.getItem(this.opts.totalNumberOfItems - 1) }; this.getTitle = function () { return this.$el.children(this.opts.titleSelector) }; this.fixIndex = function (b) {
                0 > b ? b += this.opts.totalNumberOfItems : b >= this.opts.totalNumberOfItems &&
                (b -= this.opts.totalNumberOfItems); return b
            }; this.getCurrentItem = function () { return this.getItems().eq(this.opts.currentIndex) }; this.goTo = function (b) { var c = this, d = setInterval(function () { c.opts.currentIndex == b ? clearInterval(d) : c.opts.currentIndex < b ? c.goToNext() : c.goToPrevious() }, c.opts.animationDuration) }; this.goToNext = function () { this.changeItem(!0) }; this.goToPrevious = function () { this.changeItem(!1) }; this.setEvents = function (b) {
                function e(a) {
                    if (Object.keys) return Object.keys(a).length; var b = 0, c; for (c in a)++b;
                    return b
                } function d(h) {
                    if (!("mousemove" == h.type && 0 == e(a.opts.eventsSettings.lastXYById))) {
                        h.preventDefault && h.preventDefault(); h.preventManipulation && h.preventManipulation(); h.preventMouseEvent && h.preventMouseEvent(); for (var q = h.changedTouches ? h.changedTouches : [h], t = 0; t < q.length; ++t) {
                            var m = q[t], p = "undefined" != typeof m.identifier ? m.identifier : "undefined" != typeof m.pointerId ? m.pointerId : 1, f = m; if ("undefined" == typeof f.pageX) if (f.pageX = f.offsetX + documentToTargetDelta.x, f.pageY = f.offsetY + documentToTargetDelta.y,
                            f.srcElement.offsetParent == b && document.documentMode && 8 == document.documentMode && "mousedown" == f.type) f.pageX += f.srcElement.offsetLeft, f.pageY += f.srcElement.offsetTop; else if (f.srcElement != b && !document.documentMode || 8 > document.documentMode) { for (var r = -2, s = -2, l = f.srcElement; null != l; l = l.parentNode) r += l.scrollLeft ? l.scrollLeft : 0, s += l.scrollTop ? l.scrollTop : 0; f.pageX = f.clientX + r; f.pageY = f.clientY + s } f = m.pageX; m = m.pageY; if (h.type.match(/(start|down)$/i)) {
                                s = r = a.opts.eventsSettings.moved = 0; for (l = b; null != l; l =
                            ...
                            0)); else if (a.opts.eventsSettings.lastXYById[p] && h.type.match(/(up|end|cancel)$/i)) {
                                ...
                                null, a.opts.busy = !1); var u = setInterval(function () { !1 == a.opts.busy && (clearInterval(u), 0 < k ? (k < a.opts.backPointPercentage ? a.getItemsContainer().animate({ left: "0px" }, a.opts.animationDuration, a.opts.animationEasing) : a.goToPrevious(), k = 0) : 0 > k && (k > -a.opts.backPointPercentage ? a.getItemsContainer().animate({ left: "0px" }, a.opts.animationDuration, a.opts.animationEasing) : a.goToNext(), k = 0)) }, 10); setTimeout(function () {
                                    c("." + a.opts.clickedElementClass).each(function () {
                                        c(this).removeClass(a.opts.clickedElementClass);
                                        (0 == a.opts.eventsSettings.moved || void 0 === a.opts.eventsSettings.moved) && c(this).click()
                                    }); null == a.opts.eventsSettings.yDirection && (a.opts.eventsSettings.moved = 0)
                                }, 100)
                            }
                        }
                    }
                ...
                else return a.opts.debug && c("<div>").html("Unexpected combination of supported features").prependTo(a.opts.debug), !1; var j, k, g, n; this.$el.data("obj", a); this.$el.trigger(a.opts.controlInitializedEventName); return !0
            }; this.changeItem = function (b) {
                if (this.opts.busy) return !1; this.opts.busy = !0; this.opts.debug && (b ? c("<div>").html("swipe left").prependTo(this.opts.debug) : c("<div>").html("swipe right").prependTo(this.opts.debug)); var e, d, a = this.getItem(this.opts.currentIndex); d = this.fixIndex(b ? this.opts.currentIndex +
    ...
    {
        init: function () { return this.$el ? (this.$el.attr("data-panorama", !0), this.prepare(this.$el), !0) : !1 }, prepare: function (e) {
            var j = c(window).height(/**/), g = this.getItemsContainer().position().top, b = j - g; e.height(/*"230px"*/b); this.getItemsContainer().height(/*"230px"*/b); this.totalNumberOfItems = this.getItems().length; var q = e.outerWidth() * (100 - this.opts.itemsGapPercentage) / 100; this.opts._item_width = q; e = Math.round(c(window).width() / 100 * this.opts.itemsLeftMarginPercent); this.opts.itemsLeftMarginPercent = e; this.getItems().each(function (d,
            a) { c(a).height("50%"/**//*b - (c(a).outerHeight() - c(a).height())*/); c(a).width("90%"/*q - (c(a).outerWidth() - c(a).width())*/); 1 == d && c(a).css({ left: q }); 1 < d && c(a).hide() }); 2 < this.opts.totalNumberOfItems && (this.getLastItem().css({ left: "-" + q + "px" }).show(), this.getItems().css("left", "+=" + e + "px"))
        }, show: function () { var e = this; this.getItemsContainer().fadeIn(function () { var j = e.getItemsContainer().position().top; e.getItems().each(function () { c(this).css("height", "-=" + j + "px") }) }) }
    }; c.fn.panorama = function (e) {
        this.length && this.each(function () {
            var j =
            new q(this, e); j.init() && (j.setEvents(this), j.show(), c(this).data("panorama", j))
        })
    }
})(jQuery); $.easing.easeOutExpo = function (c, q, e, j, g) { return q == g ? e + j : j * (-Math.pow(2, -10 * q / g) + 1) + e };